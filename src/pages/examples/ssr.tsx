...
export default function Example({ dataStr }: { dataStr: string }) {
  const [data] = useState<typeof DATA>(JSON.parse(dataStr) as typeof DATA);
  const view = useView<keyof Row>("ssr", {
    sm: {
      type: false,
      amount: false,
      category: false,
